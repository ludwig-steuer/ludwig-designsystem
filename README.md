...
| **Wann** | Jeder Export trägt im JSDoc `@when` (der Fall, für den er da ist) und `@instead` (der Nachbarfall und wohin der gehört: `… → MasterDetail`). Das ist die Antwort auf „was nehme ich?" — greppbar und im Editor-Hover. |
