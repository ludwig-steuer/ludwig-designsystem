...
- Notes: Unabhängig von `doc_direction`. Ein und derselbe `document_kind` kann mit `inbound` ODER `outbound` kombiniert werden — die Felder beantworten unterschiedliche Fragen. **Buchhalterische Tabelle** (Spalten: `document_kind` × `doc_direction`):
  ...
  Das deutsche Wort „Gutschrift" ist mehrdeutig: Storno-Gutschrift = `credit_note`, §14-UStG-Selbstabrechnung = `self_billing`, Rückzahlung ohne Storno-Bezug = `refund`. Eine `credit_note` oder `self_billing` **ist** ein Rechnungsdokument: steht daneben eine Zahlungsbeleg-Form, korrigiert `reconcile_form_with_kind` die Form auf `commercial_invoice` (`belege.md` R7b) — `refund` bewusst nicht, eine Erstattung kann legitim auf einem Kontoauszug stehen. Der Classifier-Prompt entscheidet anhand Vorzeichen + Wortlaut + Aussteller-Rolle + periodisches Abrechnungs-Schema. `reimbursement` (Auslagenersatz / Reisekosten) wird hier NICHT modelliert — Signal lebt in `DocumentForm.expense_report`. Stand 2026-05-14 sind im Booking-Modul nur `original` und `self_billing` als Buchungs-Pfad implementiert; `credit_note` und `refund` parken auf Review, bis der Vorzeichen-Flip-Pfad steht.
...
- Notes: DATEV-exception. `posting_text` / `narrative` would be valid English, but the DATEV term is so universally used in the Kanzlei domain that Anglicizing just adds translation overhead. Der Text hängt an der **Teilbuchung**; ein Satz-Kopffeld gibt es nicht. Der Agent muss ihn am Stil der Konto-Historie ausrichten (`search_past_bookings(accountNumber=…)`), der Judge prüft und korrigiert das (B8); bei Personenkonten setzt ihn der Server aus dem Spiegel (`lineTextDefault`, F157). Mandanteneigene Konvention: `posting_text_convention` am Client-Profil.

### Provision (Rückstellung)

- English: `provision`
- German: `Rückstellung`
- Definition: Bestandskonto für eine dem Grunde nach bekannte, der Höhe oder Fälligkeit nach ungewisse Verbindlichkeit (SKR03 `0950–0979`, SKR04 `3000–3095`). Die Bildungsbuchung „Aufwand an Rückstellung" erfasst den Aufwand im Entstehungsjahr; die spätere Zahlung tilgt die Rückstellung und ist **kein** neuer Aufwand.
- Data type: kein eigenes Feld — ein Konto in `ludwig.client_ledger_accounts`. Erkannt am Namensstamm `rückst` (case-insensitiv), nicht über `skr_class`: dieselben Konten liefern in SKR03 `fixed_assets`, in SKR04 `liabilities`, weil das Feld dem Nummernblock folgt.
- Example: `0963 Körperschaftsteuerrückstellung`, Haben-Bestand 193.543,40 € → eine Zahlung „KOERPST 2024" darüber tilgt die Rückstellung.
- Notes: Code-Bezeichner ist durchgängig `provision` (`paymentMaySettleProvision`, `provisionNameTokens`, `B7_PAYMENT_MAY_SETTLE_PROVISION`). Der Namensstamm allein trifft auch Aufwands-, Ertrags- und Vortragskonten mit „Rückstellung" im Namen — der Guard schließt sie aus (buchung.md R13g).
...
- Definition: Ein **vom Mandanten selbst gebuchter** EXTF-Buchungsstapel (Rechnungssoftware, Kasse, Warenwirtschaft), den Ludwig entgegennimmt und als Entwurf importiert (`client_journal_entry.origin='client_import'`), statt dieselben Umsätze nachzubuchen. Nicht zu verwechseln mit dem **DATEV-Import** (was in DATEV *ist* → `client_datev_mirror_entries`) und nicht mit dem **Export** (was Ludwig *nach* DATEV schickt): der Mandantenstapel ist das, was noch nach DATEV *soll*. Seit F163 ist er ein **eigener Buchungszyklus** (`client_datev_export_batches.kind='client_batch'`, Benennung `MM-YYYY-EXT[-Kasse|-Debitoren]`), den der Agent in einem eigenen *Durchgang* der Art `client_batch` **vor** dem regulären Stapel bearbeitet — dort wird nichts gebucht, nur Belege werden den importierten Sätzen zugeordnet (Teilschritte 1b · 1d · 1e · 1f · 2a · 5a).
- Data type: `ludwig.client_batch_imports` (ein Lauf je Datei: Datei-Hash als Idempotenz-Anker, `rejections` als Ablehnungsprotokoll, `export_batch_id` = der eröffnete Zyklus) + `client_journal_entry.batch_import_id`/`import_reference`/`export_batch_id`/`source_doc_id` (der Beleg zu genau diesem Satz — daraus schreibt der Export den DATEV-Beleglink). Modul `apps/web/src/modules/client-batches/` (Parser `domain/extf-batch.ts` pure, Import `application/import-client-batch-core.ts`).
...
- Notes: Der Import legt **keine** fachlichen Sachverhalte an — alles geht in einen Sammel-Sachverhalt, den der *Sachverhalts-Zuschnitt* (nächster Eintrag) nachgelagert über den Bestand formt. Erkennung am EXTF-Kopfsatz (Kategorie 21/16), kein Setting davor; `platform_clients.expects_client_batches` setzt der erste Import selbst; es sagt nur noch aus, dass dieser Mandant Stapel liefert (seit 2026-09-09, Befund 73/74). Gate 1e hängt seit F164 am Mandantenstapel-Durchgang selbst und liest dort dessen Import-Lauf. Parsen ist **deterministisch, nie LLM** (Beträge/Kontonummern/BU dürfen nicht durch Transkription laufen). Die Sätze durchlaufen die normale Kanzlei-Abnahme, aber **keinen Judge-Pass**: der Mandant hat gebucht, die Kanzlei nimmt ab. Sie sind vor der Abnahme als OPOS/Präzedenz sichtbar (eigener Zweig in `client_effective_journal_lines`) und werden von `submit_booking_proposal` nie gelöscht. Ein Satz, dessen Konten nicht sauber auflösbar sind, wird **abgelehnt und protokolliert** statt geraten. **Stammdaten-Sperre** (F165): fehlt zu einem Satz das Personenkonto (unbekannt oder nur Platzhaltername), ist der Stapel bis zur Debitoren-/Kreditorenliste **nicht freigebbar** — die Belegzuordnung läuft weiter, und der Server zieht blockierte Läufe nach, sobald die Liste (oder ein weiterer Stapel) die Konten liefert. Introduced 2026-08-11 (F69); eigener Zyklus + Beleg je Satz 2026-09-09 (F163–F165).
