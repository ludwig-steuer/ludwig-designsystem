...
- `status/` — `status-registry.ts` + `StatusBadge` (R1/R2).
- `components/log` — `LogTable`/`LogView` + Zell-Bausteine (R7).
...
selbst zusammenzusetzen (R15). Für Buchungen gibt es bewusst zwei Inhalte
...
(R18-Kern konsolidiert auf einen Satz), Notizen sind Audit-Einträge und
