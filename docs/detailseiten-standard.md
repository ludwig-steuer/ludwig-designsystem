...
| 3 | **Signal** | wenn es etwas zu melden gibt, das den ganzen Datensatz betrifft | die eine Meldung über allem: „wird eingeordnet", „Einordnung fehlgeschlagen"; hier steht auch der nächste Schritt (§4.1); nichts, wenn der Datensatz auf eine andere Partei wartet (D22) und, bis der Kopf einen Platz dafür hat, das Prozessbild (Z7) | `StatusCallout` (0049) · `Banner` |
...
| wartet auf eine andere Partei (Mandant, DATEV, Agentendurchgang) | nirgends als Satz — Zustand und Staffelstab im Kopf, die Erklärung hinter dem (i) (D22) | — |
...
| **D22 Ein Signal oder keins** | Der Signal-Slot trägt höchstens eine Meldung. Wartet der Datensatz auf jemand anderen — den Mandanten, DATEV, den Agenten —, steht dort nichts: die Wartezeit ist eine Erwartung oder ein Zustand im Kopf, keine Aufforderung an die Leserin. **Abgrenzung (2026-09-25, Anlass Stapelseite F302):** Zwei Fälle stehen im Signal: der nächste Schritt der Leserin (ein Knopf, §4.1) und ein Fehler, der den ganzen Datensatz betrifft. Dazu kommt ein Vorgang, der **fertig wird, während sie hinsieht** („wird eingeordnet — die Seite aktualisiert sich selbst“, `info`); das ist Systemrückmeldung, kein Warten. Wartet der Datensatz auf eine andere Partei (Mandant, DATEV, ein Agentendurchgang), steht **weder** ein Signal **noch** ein Satz im Kopf. Das Warten zeigen der Zustand und der Staffelstab, die Erklärung (warum, bis wann, was danach) steht hinter dem (i) des Zustands (§3.2, Z3). | Zwei Banner übereinander; „Warten auf Unterlagen" als Signal mit Knopf, obwohl im Haus niemand etwas tun kann. | `StatusCallout` (0049), `Banner` |
