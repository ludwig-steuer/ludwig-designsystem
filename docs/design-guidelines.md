...
| **T7 Zahlen und Zeit** | `de-DE`: `1.234,56 €`, immer zwei Dezimalstellen, `tnum`, Vorzeichen ohne Farbe (A7), Soll/Haben als S/H; nur `fmtMoney` (`@/ui/booking/format`) / `formatMoney` (`shared/money.ts`). Datum `TT.MM.JJJJ`, Zeit `HH:MM`, immer `timeZone: "Europe/Berlin"` (R3); relative Zeit („vor 3 Tagen") nur **neben** der absoluten, nie allein (F123 §2.4). Zähler: Zahl vor Wort — „3 offen", „12 von 47". **In einer Tabellenzelle** nur über die Wertzellen (`AmountCell`, `CountCell`, `PercentCell`, `Timestamp`, `DateRangeCell`, `BooleanCell`, `IbanCell`; 0197/0199) bzw. ihre Spaltenfabriken (`amountColumn` …, 0198), außerhalb über `Amount`/`Time` oder die `format*`-Funktionen aus `src/ui/v3/format.ts`; ein Hinweis am Wert über `hint`. | Lokaler `fmtDate`/`fmtEUR`, `toLocale*` in einer Zelle, `align: "end"` von Hand an einer Zahlenspalte, `? "Ja" : "Nein"`, `TimeAgo` als einzige Zeitangabe. |
| **T8 Beschriftung** | Jedes Icon hat ein Wort (V7) — das Wort steht in der Registry am Eintrag (`label`), damit es überall dasselbe ist (0087); jede Taste steht an ihrer Handlung (`KeyButton`, V14); Tooltip erklärt, ersetzt kein Label; Placeholder ist kein Label; Legende am Ort (`StatusHeader`). **Eine benannte Ausnahme** (Owner, 2026-09-03, Aufgabe 0012): `IconButton` darf ohne sichtbares Wort stehen, wenn **alle drei** gelten — (1) die Handlung ist konventionell und ohne Vorwissen erkennbar (schließen, blättern, auf-/zuklappen), (2) sie ist umkehrbar und folgenlos, (3) sie ist nicht der einzige Weg: ein beschrifteter zweiter existiert. `label` bleibt Pflicht und wird `aria-label` **und** `title`. Kebab-Menüs fallen nicht darunter — die gehen an `OverflowMenu` mit sichtbarem Wort. | Icon-Only-Button ohne sichtbares Wort **außerhalb dieser drei Bedingungen**; Icon-only für eine schreibende Handlung; Kebab; Legende in `/hilfe`. |
| **T9 Typografische Zeichen** | Erlaubt als Text: · – — … „" %. Häkchen, Kreuze, Pfeile, Warnzeichen, Punkte als Träger von Bedeutung nur als Lucide-Icon — und zwar über die Registry (`ACTION_ICON`, `ENTITY_ICON`, 0087), damit dasselbe Zeichen überall dasselbe heißt. | ✓ ✗ ⚠ ● → in Buttons, Badges, Leerzuständen, Tabellenzellen; ein Zeichen direkt aus `lucide-react` in einem Baustein. |
...
| **I3 Tabs** | `TabBar` (R9) mit Zähler/Alarm; Inline-`borderBottom`-Kopien sind Verstoß (P8). **Ein Reiter ist eine Sicht, kein Filter** (Owner-Entscheid 2026-09-08, ausführlich unter R9 und D10): unterscheiden sich zwei Reiter nur in der Grundgesamtheit, sind sie ein verkleideter Filter — nicht kombinierbar, nicht neben den anderen Filtern in der URL, und sie behaupten einen Ortswechsel, wo nur ausgewählt wird. Dass ein Reiter „nie die Optik wechselt" (R17) gilt weiter für die **Leiste**: gleiche Höhe, gleiche Ordnung, kein Bruch mitten in der Reihe. Für den **Inhalt** gilt es nicht — eine Sicht darf andere Spalten und eine andere Form haben, sonst wäre sie keine. **Wann Reiter, wann Schnellfilter (0201):** Reiter nur für Mengen, die sich nicht überschneiden und zusammen alles ergeben (MECE, etwa Eingang · Ausgang), oder für Sichten mit eigener Form (andere Spalten, anderes Bauteil). Überschneiden sich die Mengen — „Offen" enthält „In Verarbeitung" —, sind es Schnellfilter (F8). Andere Sortierung oder ein weggelassener Zeitraum machen noch keine eigene Form: beides ist Filterstand. *(Der Satz „ein Tab filtert eine Quelle" stand hier bis 2026-09-08 und sagte das Gegenteil dessen, was heute gilt.)* |
| **I4 Filter** | Eine `FilterBar` statt Varianten pro Seite; Regel 0200: **F1** sofort wirksam (Klick, bei Text nach kurzer Pause; im Server-Formular `autoSubmit`) — ein Knopf „Filtern" nur bei spürbar teurer Abfrage, im Code begründet · **F2** „Zurücksetzen" steht immer da, gesperrt, solange nichts gesetzt ist · **F3** „Gefiltert: x von y" rechts in der Leiste (`result`), gezählt mit dem Filter der Liste (I12) · **F4** bis etwa 5 exklusive Werte oder ein Status → `FilterChips` mit Zahl je Option; mehr Werte oder mehrere zugleich → `MultiSelectFilter`; Zeitraum → `PeriodField`/`DateRangeField`; Freitext → `SearchInput` · **F5** der Stand steht am Filter selbst, keine zweite Zeile „aktive Filter" · **F6** leer nach Filter nennt den Filter und den Weg zurück (T6) · **F7** Filterstand in der URL; keine still vorgewählten Filter (V14, V9) · **F8** benannte Filterstände (Schnellfilter) als eigene Zeile über den Feldern (`FilterBar presets`, `FilterChips label="Schnellfilter"`). Ein Schnellfilter setzt nur die Felder darunter, er ist kein zweiter Filter. Markiert ist er genau dann, wenn die Felder exakt seinen Stand tragen (`matchPreset`). Sein Zähler zählt mit demselben Prädikat wie die Liste (I12). Vorlage: `DataTable` → `FilteredListTemplate`. Verstöße: Filter, die ohne Grund erst nach „Filtern" wirken; ein Zurücksetzen, das verschwindet; Chips für 15 Werte; ein Dropdown für „Alle / Offen"; Reiter für Mengen, die sich überschneiden. |
...
| **I11 Die Zeile ist das Ziel** | Hat eine Listenzeile ein Detail, führt die **ganze** Zeile dorthin, nicht nur ein Wort: `Row href=…` (`@/ui/v2`). Trägt die Zeile daneben eigene Links oder ein Zeilenmenü, deckt stattdessen der Link auf das Detail die Zeile ab — Klasse `v2rowlink`, kein `<a>` in `<a>`; die übrigen Links/Knöpfe liegen darüber und behalten ihr eigenes Ziel. So bleibt genau **ein** Fokus-Stopp für das Ziel, mit eigenem Text statt `aria-label`. Hover färbt die ganze Zeile (§2). Eine Zeile ohne Detail bleibt stumm — kein Cursor, kein Hover. **Zwei Ziele, zwei Wege:** führt die Zeile in die Vollansicht **und** gibt es daneben einen Drawer, bekommt der Drawer ein eigenes Zeichen — `ActionIcon action="peek"` als `RowAction` mit `href` auf den Such-Parameter (L3), rechts in der Aktionsspalte. Führt die Zeile ohnehin nur in den Drawer, steht dort **nichts**: ein Zeichen neben einem Klick, der dasselbe tut, ist Rauschen (Owner-Entscheid 2026-09-08). |
| **I12 Die Zahl zählt, was hinter ihrem Weg steht** | Eine Kachel oder ein Zähler mit `href` zählt **mit exakt dem Filter, den die Liste dahinter anwendet** — sonst zeigt die Kachel 225 und der Klick 180, und die Zahl war eine Behauptung. Der Fall ist real: „offene Rückfragen" stand auf 72, weil gegen `answered_at is null` gezählt wurde, während die Liste `clarificationIsOpen()` anwendet — es sind 59 (Mandantenjahr-Profil, 2026-09-08). **Praktisch heißt das: dieselbe Ableitung auf beiden Seiten, nicht dasselbe SQL zweimal geschrieben.** Eine Zahl ohne Weg darf abweichen, muss aber sagen, was sie zählt; eine Zahl **mit** Weg darf es nicht. Gilt für `KpiTile href`, Reiter-Zähler und jeden Zähler in einem Kopf. |
...
| **A8** | **Icon-Leiter je Register:** produktiv 12/14/16 px, lesend 16/20/24 px, Stroke 1.5. Das Icon folgt der Schriftstufe seines Registers; 16/20/24 aus dem Marken-Brief galt dem lesenden Register mit 16-px-Text. **Welches Zeichen** — das sagt seit 0087 die Icon-Registry `src/ui/v3/Icons.tsx` (Grundlagen-Ebene, neben `format.ts` — nicht in `patterns/`, weil acht Primitives sie brauchen): `ENTITY_ICON` je Ding, `ACTION_ICON` je Bedeutung, dazu `EntityIcon`/`ActionIcon`, die Maß und Strich selbst setzen (`size` nimmt nur die Werte der Leiter). Ein Baustein importiert kein Zeichen mehr direkt aus `lucide-react`; `pnpm check:icons` meldet jeden, der es doch tut. Abweichler (12.5, 13, 15; Stroke 1.75, 2) ziehen mit dem Umzug auf die Leiter. |
...
| | Owner 2026-09-04 |
| **A10** | **Jeder Drawer trägt den Weg in die Vollansicht** — immer, und immer an derselben Stelle: als einziger Knopf im `footer` des `Drawer` (Zone 5 des Zonen-Schemas, 0052) — und zwar **`DrawerFullView`**, „Vollansicht öffnen", in jedem Drawer gleich (0187). Ein Drawer ohne diesen Knopf ist unfertig, nicht „minimal". |
| **A11** | **Eine Entität in zwei oder mehr Listen bekommt eine Spaltenfunktion**, kein Tabellen-Wrapper: `<entity>Columns(opts)` in ihrem `entities/`-Ordner liefert `ColumnDef[]`, jede Liste baut daraus, keine schreibt Spalten selbst. Grund: ein Wrapper um `DataTable` müsste deren 16 Props durchreichen und wächst mit jedem Aufrufer, der `selection`, `filtered` oder `next` braucht — die Funktion gibt dieselbe Garantie („überall gleich“) ohne Zwischenschicht und ist auch dort verwendbar, wo `DataTable` nicht passt. Ausprägungen (schmal/breit) sind ein Argument der Funktion, keine zweite Komponente. Bei **einer** Liste steht die Definition in der Aufrufstelle; die Schwelle ist dieselbe wie die Zwei-Verwendungen-Regel für neue Primitives (`spec-schreiben` §3). Kein `DataTable` im Namen — das ist die Implementierung, nicht die Sache. |
| **A11a** | **Wann überhaupt `DataTable`:** Spalten + Sortierung + Seitenwechsel → `DataTable`. Titel, Zustand und eine Meta-Zeile, aufklappbar → eigene Komponente (Präzedenz: `ClarificationList`). **Nicht** im Drawer: `DataTable` führt Seite, Größe und Sortierung über die URL (0057 E1/E2), und ein Drawer kennt das Routing nicht (0052) — dort trägt eine nackte `Table` dieselben Spalten, mit „Mehr laden“ als Client-Insel. |
| **A12** | **Schnittstellen wachsen additiv, nicht auf Vorrat.** Eine Prop, die es gibt und die nichts tut, wird nicht gebaut — der Aufrufer sieht ihr nicht an, dass sie ignoriert wird, und der Erste, der sie benutzt, meldet einen Fehler. Drei Regeln machen eine Schnittstelle langlebig: (1) **Das VM bildet die Tabelle ab, nicht die Ansicht** — `ClarificationVM` heißt wie die Spalten (`raisedAt`, `answeredAt`, `deferredUntil`), dann wächst es mit dem Datenmodell und ein Redesign fasst es nicht an; eine Prop, die eine Gestaltungsentscheidung trägt (`badgeRechts`), ist beim nächsten Entwurf falsch. (2) **Erweitern heißt: optionale Prop dazu** — rückwärtskompatibel, kein Aufrufer bricht. (3) **Verhalten kommt als optionaler Callback, nicht als Boolean** — `onDefer?` statt `canDefer`: fehlt der Callback, fehlt der Weg, und „gibt es noch nicht“ und „hier nicht erlaubt“ sind derselbe Zustand (Präzedenz: `onResolve`, `renderDetail`). Geplante Ausbaustufen stehen im Abschnitt **„Ausbau“** der Spec — nicht als tote Prop und nicht als Code-Kommentar, der still veraltet. |
