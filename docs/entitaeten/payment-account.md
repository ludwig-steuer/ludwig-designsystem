...
| Auszugserwartung (`statement_expectation` · `statement_expectation_manual`, seit F235 dreistufig: Pflicht · Sollte kommen · Keine; `expects_statements` bleibt als „≠ Keine“) | Spalte · Spalte | Zustand (`statement_expectation`) | vor F235 gezählt: 4 erwartet (3 von Hand, 1 abgeleitet), 3 „keine (Hand)" | `bankkonten/page.tsx` (`StatementExpectationCell`) | Server · Nutzer (Hand) | 6 | S | Füllgrad · GLOSSARY (Gate 1a) |
