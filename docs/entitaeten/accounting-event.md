...
- **L-309** Die Spiegelbuchung als Quelle fehlt in den Formen des Ereignisses: `TimelineEventVM.source` kennt nur `"doc" | "bank"`, und `CaseEvent` trägt `sourceDoc` und `bankTransaction`, aber keine Spiegelbuchung — obwohl 183 Vorträge eine haben. `deriveTitle()` liegt in `application/case-overview.ts` und wird nicht gespiegelt.
- **L-310** Belegereignisse mit Beleg tragen den Betrag 0 (499 von 499), solche ohne Beleg den Betrag selbst (493). Wer `amount` zeigt, zeigt bei der Hälfte nichts; der Betrag steht am Beleg (`CaseEventSourceDoc.invoiceTotalValue`).
