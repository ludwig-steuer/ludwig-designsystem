...
| `AccountView` → **`LedgerAccountView`** | L | **gebaut (0063)** | 1 — die Kontoseite existiert. Der Name musste weichen: `AccountView` ist in `domain/account.ts` bereits ein Typ (`"flat" | "grouped"`) | alles ab 20 % Füllgrad | alle | `AccountFacts`, `accountEntryColumns`, `BarChart` | `accounts/[accountNumber]/page.tsx` |
