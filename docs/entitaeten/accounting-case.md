...
| Status-Achsen | `sachverhalt` (`lifecycle_status`) · `disposition` (Zuständigkeit) · `export_case` (abgeleitet) · `belegnummern_modus` (`document_number_mode`) · `klaerung` (Zähler der offenen Fragen) · `triage` (Abnahme-Bucket, `acceptance-triage.ts`) · `ereignis` und `erwartung` an den Kindern |
...
| Prüfung von / am | Claude (zweiter Agent), 2026-09-05 — Zahlen gegen denselben Bestand nachgerechnet, jede Zeile mit Beleg „heute in" gegen die genannte Datei gelesen. Geändert: Rang des Gegenparts, drei Zeilen der heutigen Darstellung, zwei Listen, drei Formen-Begründungen |
...
|---|---|---|---|---|---|---|---|---|
| Anzeigename | `abgeleitet: fehlt → Befund L-52` — `title`, sonst `<Art>: <Gegenpart>`, sonst `<Art>` | Identität | `title` 53 %, Rückfall greift bei 47 % | `PortalCaseList` (Z. 255–256) als einzige Stelle mit genau dieser Kette; `Hero` über `buildCaseOverview()` (`overview-vm.ts` Z. 708–710) mit einer **anderen** Kette — `<Art>: <Gegenpart aus den Ereignissen>`, `title` wird nie gelesen; `CasesTab` des Geschäftspartners mit einer dritten (`title ?? summary ?? „—"`); die Sachverhaltsliste zeigt ihn gar nicht, nur im Tooltip | Nutzer (`title`) | 1 | XS | nachgelesen 2026-09-05 in allen vier Dateien · JSDoc an `CaseListItem.title` beschreibt die Kette, ohne sie zu implementieren. **Die Kette ist nicht dreimal gebaut, sondern dreimal verschieden** — der v3-Anzeigename ist die eine Regel, nicht die Mehrheitsmeinung |
...
| Nummer (`caseNumber`) | Spalte, Trigger `assign_case_number` | Identität | 100 % | `CaseCell` (2 fremde Listen), die dritte hat eine eigene Kopie (`KontoauszugView`), Listen-Spalte „ID", `Hero`, `CloseCasesPanel`, `CasesTab` des Partners | nie | 3 | XS | die einzige Angabe, die `CaseCell` heute trägt |
...
| Gegenpart (`counterpartyName`) | Spalte | Identität | 95 % (p50 14 · p90 27 · max 57 Zeichen) | Listen-Spalte „Gegenpartei" (sichtbarer Zellentext), `Hero` (zweimal: Meta und Fakt), `CloseCasesPanel` mittelbar über das Personenkonto | Nutzer | 5 | **S** | Füllgrad · heute eine eigene Spalte der Zeile. **Korrigiert am 2026-09-05 (war Rang 10, ab M):** die Begründung „in der Zeile trägt ihn Rang 1 schon" hält nicht — von 915 Fällen haben 483 einen `title`, und in **467** davon (51 % aller Fälle) kommt der Gegenpart im `title` nicht vor (`position(lower(counterparty_name) in lower(title))=0`). Ohne eigene Spalte verlöre die Hälfte der Zeilen die Angabe, mit wem der Vorgang läuft. Seitenprofil Zweifel 4 („steht dreimal") gilt dem **Kopf**, dessen Titel den Gegenpart per Ableitung immer enthält — nicht der Zeile |
| Zuständigkeit (`disposition`) | Spalte, Achse `disposition` | Verantwortung | 95 % (`agent` 622 · `accounting` 246 · NULL 47; `client` **0**) | Listen-Spalte „Zuständig" | Nutzer (`CASE_DISPOSITION_WRITABLE`: agent, accounting) | 6 | S | Seitenprofil Rang 2 („bin ich dran?") · eigene Registry-Achse |
| Art (`kind`) | Spalte, `CASE_KIND_LABEL` — **kein Status** | Identität | 100 % (`outgoing_invoice` 388 · `incoming_invoice` 307 · `recurring_charge` 104 · `adjustment_only` 91 · `internal_transfer` 16 · `expense_report` 7 · `contract` 2) | Listen-Spalte „Art", `Hero` (`CaseKindEditor`), Portal-Meta, Auswahl-Dialog, `CloseCasesPanel`-Karte | Nutzer (`CaseKindEditor`) | 7 | S | Füllgrad · fünf Komponenten · Filter `recurringMode` |
| Offene Klärungen | `abgeleitet: openClarificationsCount` (`clarificationIsOpen`) | Zustand | 92 % = 0 · max 3 | Listen-Spalte „Klärung" (Pille „n offen") | Server | 8 | S | heute eine eigene Spalte · Achse `klaerung` |
| Eröffnet (`openedAt`) | Spalte | Zeit | 100 % | Listen-Spalte „Eröffnet", `Hero`-Fakt, Portal-Meta, `CasesTab` des Partners | nie | 9 | S | Füllgrad · Sortier-Vorgabe der Liste (`caseOrderExprs` = `opened_at desc`, für alle vier Reiter dieselbe) |
| Export-Zustand | `abgeleitet: deriveCaseExportStatus()`, Achse `export_case` | Zustand | 7 % haben `export_batch_id`; die Ableitung zählt Buchungen, nicht die Spalte | Listen-Spalte „Export" | Server | 10 | S | heute eine eigene Spalte · Ableitung liegt im Spiegel |
| Zusammenfassung (`summary`) | Spalte | Erklärung | 75 % (p50 231 · **p90 309** · max 720) | `Hero` (`CaseSummaryEditor`), Portal-Karte, `CloseCasesPanel`-Karte, Tooltip der Liste, `CasesTab` des Partners (als Rückfall auf den Anzeigenamen), `<select>` der Bankzuordnung (40 Zeichen); `CaseOverviewBox` zeigt sie ebenfalls, wird aber **nirgends gerendert** | Nutzer | 11 | M | Füllgrad · fünf gerenderte Komponenten. **Kürzen ab 160 Zeichen** in M, voll in L (p90 309 passt in keine Karte) |
| Geschäftspartner (`counterpartyPartnerId`) | Spalte → `client_business_partners` | Kontext | 47 % | `Hero` (Link „Zum Kreditor") | Server | 12 | M | heute ein Link im Kopf. `CaseFilter.counterpartyPartnerId` existiert, ist aber **kein Filter der Sachverhaltsliste**, sondern die Grundgesamtheit des Reiters „Sachverhalte" auf der Geschäftspartner-Seite (geprüft 2026-09-05) |
...
| Kein Beleg zu erwarten (`documentNotRequiredReason`) | Spalte | Erklärung | 7 % (p50 116 · p90 176 · max 313) | `FehltPanel` und `BelegeTab` (dort als `EmptyState`-Untertitel) | Nutzer | 15 | M | Füllgrad < 20 %, aber das **Fehlen bedeutet etwas** (Spaltenkommentar) — deshalb eigener Punkt, nicht vor M |
...
| Anker (`batchOposReference`) | Spalte | Kontext | 47 % | nirgends | nie | 18 | L | Füllgrad. Sagt, **woher** der Fall kommt — die einzige Antwort auf „warum gibt es diesen Fall?" bei automatisch gegründeten Sachverhalten. Der Spaltenkommentar nennt drei Formen, im Bestand kommen zwei vor: `mirror-opos:` 426 · `payment-collect:` 2 · `mirror-opos-pool:` **0** |
...
| Verrechnungskonto (`fyClearingAccountId`) | Spalte → `client_ledger_accounts` | Kontext | **0 %** (2 von 915 Zeilen) | `Hero` (Konto + Saldo „ausgeglichen"/„Rest") | Server | 22 | L | gerundet 0 %, tatsächlich zwei Zeilen; im Kopf gebaut (F104) — im Bestand kaum angekommen. Der CHECK schließt `fy_personal_account_id` aus (XOR) |
...
| Abnahme-Bucket | `abgeleitet: triageCaseBucket()` (`acceptance-triage.ts`), Achse `triage` | Zustand | nur für Fälle mit offenem Vorschlag | Gruppenkopf in `CloseCasesPanel` (`prüfen` · `kurz ansehen` · `durchwinker` · `übernehmen`) | Server | 25 | L | **nachgetragen am 2026-09-05:** eigene Registry-Achse `triage`, im Profil bisher nur unter „Typen" erwähnt. Gehört zur Abnahmeliste, nicht in die Zeile — er sagt nicht, was der Fall ist, sondern wie genau man hinsehen muss |
...
|---|---|---|---|---|
| `ui/case/CaseCell.tsx` (46 Z.) | XS, in **2** fremden Listen (`documents/page` Z. 282, `StuckDocumentsTable` Z. 102). Die dritte, `KontoauszugView`, hat seit jeher eine **eigene** `CaseCell` (Z. 134–172) mit anderer Schnittstelle: sie nimmt die Bankzeile, zeigt bei 0 Fällen die Pille „offen", bei 1 den Link und bei n einen Stapel aus Link + Betrag je Fall — geprüft 2026-09-05 | Nummer als Mono-Link, Rückfall auf UUID-Präfix, „—" ohne Fall | **der Zustand** (Rang 2) — wer im Kontoauszug den Fall verlinkt sieht, weiß nicht, ob er noch offen ist (Befund L-54) | Hex-freie, aber handgeschriebene Inline-Styles |
| `[year]/cases/page.tsx` (571 Z.) — Kopfzeile + 11 Spalten, `CaseIdCell` · `CaseKindCell` · `StatusCell` · `ExportStatusCell` · `DispositionCell` · `ClarificationCell` | S (Zeile) | Ränge 2–10 vollständig, dazu die beiden Kinder-Zähler | Rang 1 (Anzeigename) steht nur im **Tooltip** (`CaseSummaryTooltip`, hinter einem ⓘ), nicht in der Zeile | Gegenpart als eigene Spalte *und* im Tooltip-Titel; die Art als Badge mit Farbe ohne Achse (Befund L-53). Der Leerfall unterscheidet vier Reiter, aber nicht Filter-Treffer von leerem Vorrat |
...
| `sachverhalt/parts.tsx` → `Hero` (Z. 152–307) | L (Kopf) | Ränge 1–5, 7, 9, 11–14, 16, 22 sowie Konto-Saldo — **nicht** 6 (Zuständigkeit), 8 (Klärungen), 10 (Export) | Rang 1 kommt hier ohne `title` zustande (`overview-vm.ts` Z. 708–710): ein vom Nutzer gesetzter Titel ist im Kopf unsichtbar | Gegenpart dreimal (Titel-Rückfall, Meta-Zeile, Faktenzeile) und ein viertes Mal im Personenkonto-Namen — Seitenprofil Zweifel 4 |
...
| `CaseOverviewBox.tsx` (173 Z.) | M (Karte) — **wird nirgends gerendert**: der einzige Verweis außerhalb der Datei ist der Barrel-Export `modules/accounting-cases/index.ts` Z. 67; die Detailseite baut stattdessen `buildCaseOverview` + `SachverhaltScreen`. `ui-repraesentationen.md` führt sie trotzdem als „Detail" | Zusammenfassung mit Bleistift + Kennzahl-Streifen (Belege · Bankbewegungen · Ereignisse · Buchungsvorschläge · dominanter Gegenpart) | — | rechnet den „dominanten Gegenpart" aus den Ereignissen, obwohl der Fall eine Spalte dafür hat |
| `CloseCasesPanel.tsx` (**941 Z.**, Reiter „Zum Schließen") | M (Karte je Fall) + Liste mit Massenaktion | je Fall: Nummer als Mono-Link, Art als farbige Pille, „Einfach"/„Multi (n Belege)", Personenkonto, Konfidenz, Zusammenfassung, darunter Belege und Buchungsvorschläge; gruppiert nach Abnahme-Bucket (Achse `triage`), Auswahlkästchen je Karte, „Gruppe auswählen", „Alle bestätigen (n)" und „Alle Vorschläge übernehmen (n)" | Bearbeitungsstand und Zuständigkeit; der Anzeigename (Rang 1) fehlt ganz — die Karte beginnt mit der Nummer | die Art bekommt hier zum zweiten Mal eine Farbe ohne Achse (fest verdrahtete Hex-Werte, nicht einmal `Badge`) |
| `business-partners/ui/tabs/CasesTab.tsx` (95 Z.) | S (Zeile) in fremdem Detail | 6 Spalten: Nummer · Sachverhalt (`title ?? summary ?? „—"`) · Jahr · Betrag · Bearbeitungsstand · Eröffnet | Zuständigkeit, Klärung, Export; die Ränge 6–8 und 10 fehlen gegenüber der Hauptliste | eine zweite handgeschriebene Sachverhaltszeile für dieselbe Entität (R17) |
| `client-portal/ui/PortalCaseList.tsx` (396 Z.) | M (Karte je Fall) | Ränge 1, 3–4, 7, 9, 11: Anzeigename (mit genau der Rückfallkette aus Rang 1), Nummer, Art, Betrag, Eröffnet, Zusammenfassung, offene Fragen und Belegwünsche als Unterlisten | Zustand und Zuständigkeit — bewusst: der Mandant sieht nur seine Aufgabe | eigene Antwort-Eingabe statt `AnswerInput` (Befund L-32) |
...
**Tabellen** über Sachverhalte; `offen` (Bankzeilen) ist keine. Die vier
unterscheiden sich **nur in der Grundgesamtheit** — nachgeprüft am 2026-09-05:
`caseOrderExprs()` gibt für alle vier `opened_at desc` zurück, der Spaltensatz
steht einmal in `CasesTable`, `CaseListFilters` hängt an allen vieren gleich,
und eine Massenaktion hat keine. Nur die vier Leerfall-Texte gehen auseinander,
und Leerfall ist keines der fünf Merkmale aus §8. Nach §8 ist das **eine**
Komponente mit einem Prop, nicht vier.

**Korrigiert am 2026-09-05:** der Reiter `schliessen` ist sehr wohl eine
Sachverhaltsliste — `caseFilterForListTab()` gibt für ihn nur deshalb `null`
zurück, weil er nicht durch `listCasesForClient` geht, sondern durch
`listCloseableCasesForClient`. `CloseCasesPanel` (941 Z.) rendert **eine Karte
je Sachverhalt**, gruppiert nach der Registry-Achse `triage`, mit
Auswahlkästchen, Gruppen- und Gesamt-Übernahme. Das ist ein eigener Job, eine
eigene Grundgesamtheit und die einzige Massenaktion, die es auf Sachverhalten
heute gibt. Dazu kommt eine dritte Liste außerhalb der Route: der Reiter
„Sachverhalte" der Geschäftspartner-Seite. Beide stehen unten und beide gehen
auf den Backlog — nicht weil sie schwach belegt wären, sondern weil die fünf
Formen voll sind (§9).

| Liste | Job | Grundgesamtheit | Sortierung | Spalten (Ränge) | Filter | Massenaktion | Leerfall | Umfang p50 · p90 | Beleg |
|---|---|---|---|---|---|---|---|---|---|
| `CaseList` (ein Baustein, Reiter als Prop) | Wenn **der Vorrat eines Wirtschaftsjahres offen ist**, will **die Sachbearbeiterin** **den nächsten Fall finden, den sie selbst zur Buchung bringen kann**, damit **sie nicht 117 Fälle einzeln öffnet, um zu sehen, wer am Zug ist** | je Reiter: `laufend` = nicht geschlossen · `belege` = `waiting_for_documents` (19 von 915) · `klaerung` = zur Bearbeitung · `alle` = alles; immer ein Mandant, ein Jahr | „Eröffnet" absteigend (heute die einzige) | 1–10 | Suche (`q`), Dauer/einmalig, Belegnummern-Modus. **Kein** Geschäftspartner-Filter; der Zuständigkeits-Filter steht in `CaseListFilters`, wirkt aber nicht — `caseFilterForListTab()` liest `dispo` nicht und `CaseFilter` hat kein Feld dafür (geprüft 2026-09-05) | **fehlt in dieser Tabelle** — die Freigabe läuft je Zeile (Befund L-16); die Massenübernahme gibt es nur im Reiter „Zum Schließen" | „nichts offen" (Erfolg, mit Zahl) ≠ „keine Treffer" (Filter, mit Zurücksetzen) — heute vier Reiter-Texte, aber diese Unterscheidung nicht | offen je Mandant+Jahr **p50 86 · p90 190 · max 259**; gesamt p50 147 · p90 220 · max 262 (7 Mandant-Jahr-Gruppen) | Staging · `CASE_LIST_TABS` · `caseFilterForListTab()` |
| `PortalCaseList` | Wenn **der Mandant gefragt wurde**, will **er** **seine offenen Fragen und Belegwünsche an einem Ort beantworten**, damit **die Kanzlei weiterarbeiten kann** | `disposition = 'client'` (`portal-queries.ts` Z. 45) | offen zuerst | 1, 3–4, 7, 9, 11 + Fragen und Belegwünsche als Unterlisten | keiner | keine | „nichts zu tun" | **0 Fälle im Bestand** — kein Sachverhalt in Staging steht auf `disposition='client'`; die Karte ist durch die Komponente belegt, nicht durch Daten | GLOSSARY „Client portal" · `PortalCaseList.tsx` |
| `CloseCaseList` (Reiter „Zum Schließen") — **Backlog, hängt an `CaseCard` (0081)** | Wenn **ein Buchungslauf durch ist**, will **die Buchhalterin** **die Vorschläge in der Reihenfolge abnehmen, in der sie Aufmerksamkeit brauchen**, damit **sie nicht 34 Sätze gleichrangig durchklickt** | Fälle mit mindestens einem Vorschlag (`status='proposed'`), ein Mandant, ein Jahr | keine Sortierung, sondern **Gruppierung** nach Achse `triage` (prüfen · kurz ansehen · durchwinker · übernehmen) | Karte statt Zeile: 3, 7, 11, 13 + Belege und Vorschläge als Unterlisten | keiner | **ja** — Auswahl je Karte, „Gruppe auswählen", „Alle bestätigen (n)", „Alle Vorschläge übernehmen (n)" | „Keine offenen Buchungsvorschläge in diesem Wirtschaftsjahr" | nicht gemessen (hängt am Lauf, nicht am Bestand) | `CloseCasesPanel.tsx` · `acceptance-triage.ts` · `ui-repraesentationen.md` §1 führt sie als „Liste" |
| Reiter „Sachverhalte" der Geschäftspartner-Seite — **Backlog, gehört ins Seitenprofil `partner`** | Wenn **ein Geschäftspartner offen ist**, will **die Sachbearbeiterin** **alle Vorgänge mit ihm über die Jahre hinweg sehen**, damit **sie beurteilen kann, ob ein neuer Fall zu einem alten gehört** | alle Fälle **eines Partners**, jahresübergreifend — als Gegenpartei benannt ODER über sein bebuchtes Personenkonto (`CaseFilter.counterpartyPartnerId`) | „Eröffnet" absteigend | 1, 3–4, 20 (Jahr wird zur Spalte, weil die Liste ihr Jahr verlässt), 2, 9 | keiner | keine | „Noch kein Sachverhalt mit diesem Geschäftspartner" | Deckel 50, danach Verweis in die Hauptliste (keine `Pagination`) | `business-partners/ui/tabs/CasesTab.tsx` · `partners/[partnerId]/page.tsx` Z. 102 |
...
|---|---|---|---|---|---|---|---|
| `CaseCell` | XS | **ja** | 3 — FK-Ziel; wird in drei fremden Listen genannt (zwei rufen `ui/case/CaseCell.tsx`, die dritte hat eine eigene Kopie) | 1–3, **mit der Rückfallkette Anzeigename → Nummer → Kurz-ID** und dem Zustand als Punkt | — | `MonoCell`, `StatusBadge`, `Link` | `ui/case/CaseCell.tsx` in `documents/page` und `StuckDocumentsTable` sowie die Kopie in `KontoauszugView` (Z. 134–172). **Die Kopie ist der schwierigere Fall:** eine Bankzeile hängt an 0, 1 oder n Sachverhalten und zeigt je Fall zusätzlich einen Betrag — die Zelle braucht den Stapel und den Nullfall („offen", verlinkt in den Zuordnungs-Reiter), sonst löst sie nur zwei der drei Stellen ab |
| `CaseRow` | S | **ja** | 1 — existiert zweimal handgeschrieben (11-Spalten-Zeile der Liste, 6-Spalten-Zeile im Partner-Reiter); 2 — Kind des Geschäftspartner-Details; 6 — drei Listen-Jobs | 1–10 | Ereignisse und Klärungen als Zähler | `Row`, `CaseCell`, `StatusBadge`, `Badge`, `Amount`, `Time` | die Zeile in `[year]/cases/page.tsx` samt `CaseIdCell`, `CaseKindCell`, `StatusCell`, `ExportStatusCell`, `DispositionCell`, `ClarificationCell`, `CaseRow.tsx`, `CaseSummaryTooltip` — und die Tabelle in `business-partners/ui/tabs/CasesTab.tsx` |
| `CaseFacts` | M | **ja** | 1 — existiert **zweifach** gerendert (`Hero`-Faktenzeile, Portal-Meta; der `CaseOverviewBox`-Streifen ist eine dritte Fassung, wird aber nirgends gerendert); tragend ist deshalb nicht die Zahl, sondern 0052: der Drawer muss die Kernfakten aus **derselben** Komponente zeigen wie der View | 11–16 (+ 17–25 mit `all`) | Geschäftspartner, Personen-/Verrechnungskonto als Inline | `FieldList`, `Amount`, `Time`, `MonoCell`, `StatusBadge`, `LongText` | die Faktenzeile aus `parts.tsx` `Hero`, den Kennzahl-Streifen aus `CaseOverviewBox`. **Achtung:** die drei Fassungen zeigen heute drei verschiedene Sätze (Kopf: Eröffnet, Abgeschlossen, Personenkonto, Geschäftspartner · Box: vier Kinder-Zähler + errechneter Gegenpart · Portal: Nummer, Art, Betrag, Eröffnet) — `CaseFacts` vereinheitlicht sie, es hebt keine bestehende Liste |
...
| `CaseCard` | M | **Backlog (0081)** | 1 trifft **zweifach** zu, nicht einfach: `PortalCaseList` baut je Fall eine Karte, und `CloseCasesPanel` (941 Z.) baut die zweite — dieselbe Entität, andere Rolle, anderer Leerfall, andere Unterlisten (dort Belege und Buchungsvorschläge, hier Fragen und Belegwünsche). Damit ist die Karte **besser belegt als im ersten Schnitt angenommen**; sie bleibt trotzdem Backlog, weil sie an zwei fremden Profilen hängt (`clarification` für das Portal, `journal-entry` für die Abnahme) und die Fünf voll sind | Portal: 1, 3–4, 7, 9, 11 + Fragen, Belegwünsche · Abnahme: 3, 7, 11, 13, 25 + Belege, Vorschläge | Klärungen bzw. Buchungen als eigene Form | `Card`, `CaseFacts`, `ClarificationCard`, `JournalEntryCard` | `PortalCaseList` **und** `CloseCasesPanel` |
| `CaseList` + `CaseColumns` | L | **Backlog (0082)** | 6 — **drei** Listen-Jobs (Vorrat, Abnahme, Partner-Reiter), `DataTable` (0057) ist gebaut. Es fehlt das **Seitenprofil** für `[clientSlug]/[year]/cases`; ohne es sind Kopfzeile, Vorratszähler und Reiterlogik geraten. Die Abnahmeliste ist keine Ausprägung dieser Liste: sie zeigt Karten, keine Zeilen | wie `CaseRow`; der Partner-Reiter mit `columns`-Prop (Jahr statt Zuständigkeit/Klärung/Export) | — | `DataTable`, `CaseRow`, `EmptyState`, `Pagination` | die Tabelle in `[year]/cases/page.tsx`, `CaseListFilters`, `CaseListTabsBar`, `business-partners/ui/tabs/CasesTab.tsx` |
...
| `CasePicker` | S | **Backlog (0084)** | 3 — der Nutzer wählt einen bestehenden Sachverhalt aus: `BankTransactionAssignmentTable` Z. 378–389, heute ein `<select>` mit „Nummer · Art · 40 Zeichen Zusammenfassung". Er hängt an `CaseRow` (dieselben Ränge in der Trefferzeile) und an der Frage, wonach gesucht wird | 1–5 in der Trefferzeile (der Gegenpart ist bei 51 % der Fälle die einzige Angabe, die den `title` ergänzt) | — | `Combobox`, `CaseCell` | den `<select>` in `BankTransactionAssignmentTable` |
...
|---|---|---|---|
| `CaseCell` | **jetzt** | trägt Row, Drawer und drei fremde Listen (zwei Aufrufe + eine Kopie) | — |
| `CaseRow` | **jetzt** | trägt drei Listen; existiert heute zweimal handgeschrieben (11 Spalten in einer Seitendatei, 6 Spalten im Partner-Reiter) | — |
| `CaseFacts` | **jetzt** | trägt View und Drawer (0052 Zone 3); existiert heute zweimal gerendert und einmal als tote Fassung | — |
...
| `CaseCard` | Backlog | zwei Einsatzorte (Portal **und** Abnahmeliste), je andere Rolle, anderer Leerfall, andere Unterlisten; hängt an den Profilen `clarification` und `journal-entry`; die Fünf sind voll | `docs/backlog/0081-case-card.md` |
| `CaseList` + `CaseColumns` | Backlog | Seitenprofil für `[clientSlug]/[year]/cases` fehlt | `docs/backlog/0082-case-list.md` |
| `CloseCaseList` (Abnahmeliste) | Backlog | setzt `CaseCard` voraus; die Massenaktion hängt an L-16 (Server Actions mit `ids[]`) | zu 0081 · eigene Nummer erst, wenn die Karte steht |
| Sachverhalt-Reiter der Partner-Seite | Backlog | keine eigene Komponente: `CaseList` mit `columns`-Prop; gehört ins Seitenprofil `partner`, das es noch nicht gibt | zu 0082 |
