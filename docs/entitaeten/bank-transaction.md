...
| `BankTransactionCard` | M | **verworfen** (Owner 2026-09-08) | `SachverhaltScreen.BankPane` (`.bankcard`) ist heute eine handgebaute Karte der Position im Sachverhaltsschirm (Betrag · „Wertstellung" · Gegenkonto · Zweck · Bankkonto). Damit greift §7 Nr. 1. Entschieden (Owner 2026-09-08): `BankTransactionCell` (0100) plus `BankTransactionFacts` (0102) ersetzen sie — beide stehen und sind abgenommen. Eine sechste Form „jetzt" verbietet §9 ohnehin | | | | |
...
| `BankTransactionCard` | **verworfen** (Owner 2026-09-08) | `SachverhaltScreen.BankPane` erfüllt §7 Nr. 1, aber der Bedarf ist gedeckt: `BankTransactionCell` (0100) trägt Betrag, Datum und Zweck, `BankTransactionFacts` (0102) den Rest — beide stehen und sind abgenommen. Die App-Stelle wird damit ersetzt, ohne eine sechste Form der Familie zu bauen, die §9 ohnehin nicht zuließe | — |
