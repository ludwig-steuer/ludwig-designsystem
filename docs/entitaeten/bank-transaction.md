...
| `SachverhaltScreen.BankPane` / `.bankcard` (in `accounting-cases`) | M (Karte in fremder Ansicht) | Betrag, „Wertstellung", Gegenkonto, Zweck, Bankkonto — als Karte im Zeitstrahl des Sachverhalts | Zustand, DATEV-Haken | **im Profil bisher gar nicht verzeichnet**; eine handgebaute Karte der Position im Sachverhaltsschirm |
