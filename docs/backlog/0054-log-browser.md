...
|---|---|
| Status | Abnahme |
| Nachzubessern (Abnahme 2026-09-04) | **1.** Der einzige deutsche Kommentar der sonst durchweg englischen Datei: `LogBrowser.tsx:194–195` („Blendet die Sicht allein aus, hilft Zurücksetzen nicht — dann ist der Ausweg die Sicht, in der etwas steht.") — auf Englisch umschreiben (`CLAUDE.md`: „Code nur Englisch … Kommentare"). Sonst nichts: 20 ✓, 1 offen (App) |
...
| **Fest** — Code englisch; `@when`/`@instead` an jedem Export | `@when`/`@instead` stehen an `LogBrowser` (`LogBrowser.tsx:64–70`), der Verweis zurück in `Log.tsx:122–124` („Filtering by view, severity or search → LogBrowser"); `LogFilterState` ist ein Typ und folgt darin `LogEntry` (0053). Alle Bezeichner beider neuen Dateien englisch (`VIEWS`, `DEFAULT_VIEW_LABELS`, `SEVERITIES`, `inView`, `inSeverity`, `matches`, `emptyAfterFilter`, `wider`, `needle`, `shown`, `viewCounts`, `severityCounts`, `activeCount`, `hasLevels` / `STORY_ROWS`, `RECORD_ROWS`, `TECH_ROWS`, `BATCH`, `fromInvoiceTrace`, `fromExtractionLog`, `Confidence`, `TRACES`, `EXTRACTION`), Story-Exportnamen englisch. **Aber:** alle 21 Kommentare der Datei sind englisch — bis auf einen, `LogBrowser.tsx:194–195` („Blendet die Sicht allein aus, hilft Zurücksetzen nicht — dann ist der Ausweg die Sicht, in der etwas steht."). `CLAUDE.md` nennt Kommentare ausdrücklich. Die deutschen Zeilen in `Nav.tsx:87`, `Segmented.stories.tsx` und `v3.css` sind **kein** Verstoß: diese Dateien sind durchgehend deutsch kommentiert, und der neue `@when`-Halbsatz in `Nav.tsx:93–94` ist englisch | ✗ |
