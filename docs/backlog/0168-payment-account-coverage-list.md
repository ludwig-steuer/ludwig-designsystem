...
| Auftrag | Die Liste der Zahlungskonten eines Stapels in der Stapelabnahme: in Schritt 1 (Mengengerüst) je Konto, was im Zeitraum angekommen ist, in Schritt 4 (Bankabgleich) je Konto die Gates mit `forRelease` als aufklappbare Zeilen (F204) |
...
| Vertagt, weil | nach §8 eine eigene Ausprägung (Grundgesamtheit „Konten des Stapelzeitraums", Spaltensatz Deckung/Gate, aufklappbare Zeilen) — und die Stapelabnahme hat kein Seitenprofil: sie gehört in `docs/seiten/stapel-detail.md` (0167); ohne es sind Reihenfolge und Wortlaut der Gates geraten |
