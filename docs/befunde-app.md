...
| **L-68** | `CaseDetail` liegt in `infrastructure/case-detail-queries.ts` statt in `domain/` — der Spiegel nimmt daraus nichts — und trägt elf Felder nicht, die die Fakten des Sachverhalts brauchen: `title`, `disposition`, `openClarificationsCount`, `exportStatus`, `counterpartySide`, `batchOposReference`, `createdByKind`/`createdByLabel`, `expectedInterval`, `agentRunId`, `exportBatchId`. Dass `title` fehlt, ist die **Wurzel von L-52** | nach `domain/` heben und ergänzen (Muster L-08/L-09); bis dahin ein lokales `CaseFactsVM`, Präzedenz `AccountFactsVM` (L-13) | 0097, Freigabe 2026-09-06 |
| **L-69** | `CaseListItem.counterpartyPartnerId` fehlt — die Zeile kann den Gegenpart nicht verlinken, obwohl **47 %** der Sachverhalte einen aufgelösten Partner haben | Feld ergänzen | 0096, Freigabe 2026-09-06 |
| **L-70** | `ExpectationRow` samt `ExpectationAudience`, `DueSource` und `ExpectationDirection` liegt in `application/expectation-core.ts` | nach `modules/accounting-cases/domain/` heben (Muster L-09) | 0025, Freigabe 2026-09-06 |
| **L-71** | `DOCUMENT_NUMBER_SOURCE_LABEL` und `_STATE_LABEL` sind eine **lokale Map** in `CasePlausibilityTab.tsx:40`; `DATEV_MAX_BELEGFELD1 = 36` steht nirgends als Konstante | Labels ins Domain-Modul, die Grenze nach `core/datev/field-limits.ts`. Bis dahin nimmt `DocumentNumberField` die Labels als Prop | 0014, Freigabe 2026-09-06 |
| **L-73** | **L-05 hängt am falschen Modul:** die Altersklassen der offenen Posten gehören zu `datev-truth`, nicht zu `accounting-cases`, und `datev-truth` hat kein `domain/`. Die Union ist zeichengleich festzuschreiben: `notDue \| d1_30 \| d31_60 \| d61_90 \| d90plus`. Dazu gehört **0029 als Wartender in L-66** | `domain/` anlegen, L-05 dorthin, Union festschreiben, L-66 ergänzen | 0029, Freigabe 2026-09-06 |
| **L-74** | **Übergänge liegen nirgends als Daten.** Vier von 62 Achsen tragen einen „Übergänge:"-Block als **Kommentar** (`beleg`, `job`, `upload`, `dispatch`), ein Dutzend weitere Pfeile im Kommentar — teils Übergänge, teils bloße Regeln. Z1-Tabellen gibt es für **zwei** Achsen (`zyklus_stapel` in `docs/topics/datev.md` R19, `produktbefund` in `docs/operations/produktbefunde.md`), und die einzige Tabelle im Code, `CASE_DOCUMENT_NUMBER_MODE_TRANSITIONS`, ist vollvermascht und hat eine andere Form | Neben den Descriptor-Maps ein `STATE_MACHINES: Partial<Record<StatusAxis, { description: string; transitions: StateTransition[] }>>`; der Deckungstest prüft, dass jedes `from`/`to` ein Schlüssel der Achse ist. Die vier Kommentar-Blöcke und die zwei Tabellen lassen sich sofort übertragen — danach entstehen die Z1-Tabellen aus den Daten statt von Hand (V13) | 0069 Befund 1 |
| **L-75** | **Z1 verlangt eine Tabelle je Achse, es gibt zwei.** `StateMachine` macht die Lücke sichtbar: jede Achse ohne Übergänge zeigt die Hinweiszeile. Das ist gewollt (R16), aber es sind rund fünfzig offene Posten | je Achse eine Tabelle, sobald L-74 die Datenform hat | 0069 Befund 2 |
...
| **L-72** | Die drei Stufen von `baseline_level` haben **keine Wörter**: weder im GLOSSARY noch als Label in der Domäne. `SnapshotCard` zeigt bis dahin den rohen Wert in `MonoCell` — keine lokale Map | Wörter ins GLOSSARY, Label in die Domäne. Dazu **L-04 präzisieren**: `datev-mirror` und `datev-truth` haben kein `domain/`, der Spiegel nimmt aus beiden Modulen nichts | 0027, Freigabe 2026-09-06 |
...
| **L-79** | **Konfidenz und Dateigröße fehlen im Anzeige-Typ des Belegs.** Die Einordnung hat eine Sicherheit (Achse `konfidenz`), und die Datei hat eine Größe — beides steht am Eingang der App, keins im View-Model, das die Listen bekommen. Ohne sie bleibt in „Upload & Inbox" die Spalte Konfidenz und beim Einreichen die Spalte Größe leer. Die Größe ist dort nicht Schmuck: 25 MB je Datei ist die Grenze, an der das Einreichen scheitert | `classConfidence` und `sizeBytes` in das View-Model der Belegliste/Inbox aufnehmen | 0070 |
