...
| **L-52** | `caseDisplayTitle({ title, kind, counterpartyName })` — **ein** Weg zum Anzeigenamen des Sachverhalts: `title`, sonst `<Art>: <Gegenpart>`, sonst `<Art>` | `modules/accounting-cases/domain/case.ts` | dreimal gebaut: `PortalCaseList.tsx` Z. 254–256; das JSDoc an `CaseListItem.title` beschreibt die Regel, ohne sie zu implementieren; die Liste setzt sie über `CaseSummaryTooltip(label=counterpartyName, caseTitle=title)` ein drittes Mal zusammen. Es ist Rang 1 aller fünf Sachverhalts-Formen | `entitaeten/accounting-case.md` B1 |
| **L-53** | Entweder Registry-Achse für `client_accounting_case.kind` oder die Farbe weg | `status-registry.ts` bzw. `CaseKindCell` | `CaseKindCell` färbt `recurring_charge` als `warning` und alles andere als `info`; `CASE_KIND_LABEL` ist laut eigenem Kommentar bewusst kein Status („keine Farbe, keine Übergänge"). Beides zusammen geht nicht — der v3-`CaseRow` zeigt die Art bis dahin ohne Farbe | `entitaeten/accounting-case.md` B2 |
| **L-54** | `lifecycleStatus` in die Queries der drei Listen aufnehmen, die `CaseCell` zeigen | `documents/page.tsx`, `KontoauszugView.tsx`, `StuckDocumentsTable.tsx` | `ui/case/CaseCell.tsx` trägt nur die Nummer. Wer im Kontoauszug sieht, dass eine Zahlung an Sachverhalt 2026-0412 hängt, weiß nicht, ob der noch offen ist — Rang 2 fehlt an der kleinsten Form. Der v3-`CaseCell` nimmt ihn mit | `entitaeten/accounting-case.md` B3 |
| **L-56** | Ein Domain-Modul für die **Kontoauszugsposition** (`client_bank_transactions`): der Zeilen-Typ und die vier reinen Ableitungen `derivePurposeParts()` (SVWZ statt Rohblock), `deriveZ()`/`restOf()` (Zuordnungs-Zustand Z0–Z3), `datevMatchTitle()` (Match-Stufe → Haken) und `deriveCaseIndicators()` | `modules/bank-transactions/domain/` — der Ordner existiert nicht | alle vier liegen in `ui/` bzw. `infrastructure/` der App; `src/ludwig/` kennt die Entität gar nicht. Eine v3-Komponente kann sich dort nicht bedienen — sie definiert die Ableitungen strukturell deckungsgleich lokal | `entitaeten/bank-transaction.md` B2 |
