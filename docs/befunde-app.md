...
| **L-38** | **Beantwortet 2026-09-04 (App-Seite): Absicht, keine Subtyp-Tabellen geplant.** Kontoauszug, Kreditkartenabrechnung und Reisekostenabrechnung sind Container bzw. Deckblätter — ihre Struktur liegt an `client_bank_import_batches` / `client_bank_transactions` bzw. an den Kind-Belegen, nicht am Beleg. Von 10 `bank_statement_pdf` sind nur 4 echte Auszüge | **Offen bleibt der Satz im GLOSSARY:** der Eintrag „Source document supertype & specializations" verlangt je Belegart einen Subtyp und sagt nicht, dass Container-Belegarten bewusst keinen haben. Wer die Regel wörtlich nimmt, hält den Zustand für einen Fehler — so ist es dieser Analyse ergangen. Die App-Seite hat den Satz zugesagt | `entitaeten/document.md` B2, B12 |
| ~~**L-39**~~ | ~~`client_source_docs_contracts` ist leer~~ — **erledigt 2026-09-04 (App-Seite): kein Befund.** Alle sieben Audit-Ereignisse hängen an einem Beleg, den es nach einem Mandanten-Wipe (nach 2026-07-20) nicht mehr gibt; das Audit-Log ist Seitenkanal und überlebt das per Design. Kein Schreibpfad-Fehler | nichts zu tun. Für das Design-System heißt es: den Vertragsrenderer gegen das **Schema** bauen, nicht gegen Staging — die Tabelle war dort nie produktiv befüllt (0076) | `entitaeten/document.md` B1 |
| **L-40** | `doc_category` zu 46 % gefüllt — **teilweise beantwortet 2026-09-04 (App-Seite):** die F87-Migration ist bewusst backfill-frei. Seit 2026-08-20 fehlt die Kategorie nur noch bei `other`-Belegen (23 Stück), und das ist Absicht: **ein Container hat keine Kategorie, sie entsteht je Teilbeleg.** Die restlichen ~170 sind Staging-Altbestand Juli/August | Backfill über die Mapping-Tabelle ist deterministisch (eine Migration) und wird von der App-Seite mitgenommen. **Regel fürs Design-System:** ein Filter oder Badge über die Kategorie führt NULL als „unklassifiziert / Container", nie als Fehler oder Lücke | `entitaeten/document.md` B3 |
